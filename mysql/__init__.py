...
from settings import MYSQL_ERP
from mysql.store import StoreModel
from mysql.room import RoomModel
from mysql.pack import PackModel
from mysql.cate import CateModel
from mysql.order import OrderModel
...
class Database(object):
    ...
    def __init__(self):
        ...
        self.init_session()
        self.store = StoreModel(self)
        self.room = RoomModel(self)
        self.pack = PackModel(self)
        self.cate = CateModel(self)
        self.order = OrderModel(self)
    ...
    def init_session(self):
        for db, value in MYSQL_ERP.items():
            self.session['s'][db] = []
